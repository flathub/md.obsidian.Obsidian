...
if [[ -e "${XDG_RUNTIME_DIR}/${WL_DISPLAY}" || -e "/${WL_DISPLAY}" ]]; then
    ...
    if [[ -c "/dev/nvidia0" && "${OBSIDIAN_SKIP_NVIDIA_WAYLAND_CHECK}" -eq 0 && "${OBSIDIAN_DISABLE_GPU}" -eq 0 ]]; then
        echo "Debug: Disabling GPU acceleration to avoid potential Nvidia driver issues on Wayland. Set --env=OBSIDIAN_SKIP_NVIDIA_WAYLAND_CHECK=1 to skip this check."
        EXTRA_ARGS+=(
            --disable-gpu
        )
    fi
